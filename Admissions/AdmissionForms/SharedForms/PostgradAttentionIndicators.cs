...
{
    public partial class PostgradAttentionIndicators : UserControl, IWizard
    {
        DS_ADM_STUDataSet ds_adm_stu;

        public PostgradAttentionIndicators()
        {
            InitializeComponent();
        }
        ...
        public bool ShowView()
        {
            ...
            try
            {
                ...
                if (WizardEnvironment.State.ContainsKey(AdmissionStateItems.ShowAttention))
                {
                    bool showAttention = (bool)WizardEnvironment.State[AdmissionStateItems.ShowAttention];
                    if (!showAttention) return !SaveAttentionInfo();
                }
            }
            catch (Exception ex)
            {
