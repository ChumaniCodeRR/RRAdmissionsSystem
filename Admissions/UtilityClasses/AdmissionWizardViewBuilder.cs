...
namespace Admissions.Utilities
{
    public static class AdmissionWizardViewBuilder
    {
        ...
        #region Gadra App Wizard
        public static List<WizardView> GetGadraCaptureFormWizard(int yearchoice, string oldstuno, string Ref_no)
        {
            List<WizardView> views = new List<WizardView>();

            IWizard gadraAdmissionChecks = new GadraAdmissionCheck(yearchoice, oldstuno, Ref_no);
            WizardView wvGadraChecks = new WizardView("GadraChecks", gadraAdmissionChecks, 1);
            (gadraAdmissionChecks as GadraAdmissionCheck).AdmissionStudentValidated += new EventHandler<AdmissionStudentEventArgs>(LoadAdmissionStudent);
            views.Add(wvGadraChecks);
            ...
            IWizard schoolDetails = new SchoolDetails();
            WizardView wvSchoolDetails = new WizardView("SchoolDetails", schoolDetails, 6);
            views.Add(wvSchoolDetails);

            IWizard matricResults = new AdmissionMatricResults(false);
            WizardView wvMatricResults = new WizardView("MatricResults", matricResults, 7);
            views.Add(wvMatricResults);

            IWizard rhodesFamily = new FamilyAtRhodes();
            WizardView wvRhodesFamily = new WizardView("RhodesFamily", rhodesFamily, 8);
            views.Add(wvRhodesFamily);
            ...
            IWizard schoolHistory = new SchoolHistory();
            WizardView wvSchoolHistory = new WizardView("SchoolHistory", schoolHistory, 11);
            views.Add(wvSchoolHistory);

            IWizard additionalInfo = new AdditionalInfo();
            WizardView wvAdditionalInfo = new WizardView("AdditionalInfo", additionalInfo, 12);
            views.Add(wvAdditionalInfo);

            IWizard attentionIndicators = new AttentionIndicators();
            WizardView wvAttentionIndicators = new WizardView("AttentionIndicators", attentionIndicators, 13);
            views.Add(wvAttentionIndicators);

            IWizard wizardEnd = new WizardEnd(WizardName.GadraCaptureAdminForm);
            WizardView wvWizardEnd = new WizardView("WizardEnd", wizardEnd, 14);
            wvWizardEnd.AllowCancel = wvWizardEnd.AllowBack = false;
            views.Add(wvWizardEnd);
